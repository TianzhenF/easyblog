    ...
    <div class="top_panel">
        ...
        <el-row>
          <el-col :span="4">
            <el-form-item label="标题" prop="titleFuzzy">
              <el-input placeholder="请输入名称" clearable @keyup.enter="loadDataList" v-model="searchformData.titleFuzzy">
              </el-input>
            </el-form-item>
          </el-col>
      ...
      <Table :columns="columns" :showPagination="true" :dataSource="tableData" :fetch="loadDataList"
        ...
        <template #typeName="{ index, row }">
          <div>
            类型:<span v-if="row.type==0">原创</span><span v-if="row.type==1">转载</span>
          </div>
          <div v-if="row.type == 1">
            转载地址:{{ row.reprintUrl }}
          </div>
