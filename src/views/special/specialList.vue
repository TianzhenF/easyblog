<template>
    <div>
        <el-button type="danger" size="small" @click="showEdit('add')">新增专题</el-button>
            ...
            <el-col :span="10">
                <el-card class="box-card">
                    ...
                    <div :style="{ 'margin-bottom': '5px' }">
                        <el-alert title="红色代表草稿,绿色代表已发布/拖动文章修改排序" type="info" show-icon :closable="false"></el-alert>
                    </div>
                    <div class="organization_configuration">
                        <el-tree class="tree-panel" ref="refTree" :data="blogList" defaultExpandAll node-key="blogId"
                            :expand-on-click-node="true" :props="treeProps" :highlight-current="true" draggable
                            @node-drop="blogDrag">
                                    ...
                                    <span class="node-op" >
                                        ...
                                        <template v-else>
                                            ...
                                            <a class="a-link" href="javascript:void(0)"
                                                ...
                                                <span v-else>---</span>
                                            <el-divider direction="vertical" />
                                            <a class="a-link" href="javascript:void(0)" @click="delBlog(data)" v-if="userInfo.userId==data.userId">删除</a>
                                            <span v-else>---</span>
                                            <el-divider direction="vertical" />
                                            <a class="a-link" href="javascript:void(0)"
        ...
        <Dialog :show="dialogConfig.show" :title="dialogConfig.title" :buttons="dialogConfig.buttons"
            ...
            <el-form :model="formData" :rules="rules" ref="formDataRef" label-width="60px">
                ...
                <el-form-item label="简介" prop="categoryDesc">
                    <el-input v-model="formData.categoryDesc" type="textarea" :autosize="{ minRows: 4, maxRows: 4 }">
                    </el-input>
                </el-form-item>
            </el-form>
        </Dialog>
...
const currentCategorId = ref(null);
...
const loadDataList = async () => {
    ...
    if (currentCategorId.value == null && result.data.list.length > 0) {
        currentCategorId.value = result.data.list[0].categoryId;
        loadBlogList();
        loadDataList();
    }
    nextTick(() => {
        dataTableRef.value.setCurrentRow("categoryId", currentCategorId.value)
    })
}
//新增,修改 专题
...
//获取专题文章
const rowClick = (row) => {
    currentCategorId.value = row.categoryId;
    loadBlogList();
}
const blogList = ref([]);
const loadBlogList = async () => {
    let result = await proxy.Request({
        ...
        params: {
            categoryId: currentCategorId.value,
            showType: "1"
        }
    })
...
.organization_configuration {
    ...
    .el-tree--highlight-current .el-tree-node.is-current>.el-tree-node__content {
        // 设置颜色
        background-color: rgba(135, 206, 235,0.281); // 透明度为0.2的skyblue，作者比较喜欢的颜色 
